...
impl ParallelizationSafetyModel {
    ...
    pub fn validate(plan: &ExecutionPlan, graph: &RawSystemGraph) -> Result<(), CompilationError> {
    ...
    fn non_deterministic_error(group_id: &str, bad_sys: &str, other_sys: &str) -> CompilationError {
...
mod tests {
    ...
    use crate::scheduler::deterministic_scheduler_builder::{
        DeterministicSchedulerBuilder, PlanInspector,
    };
    ...
    fn full_pipeline(defs: &[SystemDefinition]) -> (ExecutionPlan, RawSystemGraph) {
    ...
    fn waw_in_parallel_group_rejected() {
        ...
        assert!(
            result.is_err(),
            "WAW conflict in parallel group must be rejected"
        );
    ...
    fn raw_in_parallel_group_rejected() {
        ...
        assert!(
            result.is_err(),
            "RAW hazard in parallel group must be rejected"
        );
    }

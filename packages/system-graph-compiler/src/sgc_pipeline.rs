...
impl SgcPipeline {
    ...
    pub fn compile(
        ...
        // Stage 6: Scheduling
        let plan = DeterministicSchedulerBuilder::build(
            &ordered, &report, schema_version, plan_version,
        );
    ...
    pub fn dry_run(
        definitions: &[SystemDefinition],
    ) -> Result<SgcSummary, CompilationError> {
        let graph   = GraphConstructionLayer::build(definitions)?;
        CycleDetector::detect_in_graph(&graph)?;
        let buckets = PhaseSegmentationLayer::segment(&graph)?;
        let ordered = DependencyResolutionEngine::resolve(&buckets)?;
        ...
        Ok(SgcSummary {
            ...
            systems_per_phase:       ordered.phases
                .iter()
...
mod tests {
    ...
    fn multi_phase_compiles() {
        let defs = vec![
            ...
            def("sys_post", ExecutionPhase::PostSimulation,  vec![], vec![], vec![]),
        ];
        let plan = SgcPipeline::compile(&defs, "0.1.0", 1).unwrap();
    ...
    fn pipeline_deterministic_same_input_same_plan() {
        ...
        assert_eq!(plan_1.plan_hash, plan_2.plan_hash,
    ...
    fn zombie_chase_full_pipeline() {
        ...
        assert!(PlanInspector::is_before(&plan, "AISystem",     "MovementSystem"));
        assert!(PlanInspector::is_before(&plan, "AISystem",     "DamageSystem"));
        assert!(PlanInspector::is_before(&plan, "DamageSystem", "DeathSystem"));
        assert!(PlanInspector::is_before(&plan, "InputSystem",  "MovementSystem"));

        let plan_2 = SgcPipeline::compile_and_verify(&defs, "0.1.0", 1).unwrap();
        assert_eq!(plan.plan_hash, plan_2.plan_hash,

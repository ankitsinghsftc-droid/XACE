...
mod tests {
    ...
    fn self_dependency_rejected() {
        let defs = vec![def("sys_a", PhaseEnum::Simulation, vec![], vec![], vec!["sys_a"])];
    ...
    fn waw_edge_lex_smaller_from() {
        ...
        assert!(g.edges.contains_key(&key), "sys_a (lex smaller) must be 'from'");
    ...
    fn waw_tie_break_argument_order_independent() {
        ...
        assert_eq!(keys1, keys2, "WAW direction must not depend on input order (D11)");
    }
    ...
    fn phase_order_direction_is_always_earlier_to_later() {
        ...
        for ((_from, _to), edge) in &g.edges {
            if edge.edge_type == EdgeType::PhaseOrder {
                ...
                assert!(fn_node.phase.as_u8() < to_node.phase.as_u8(),
    ...
    fn explicit_dep_beats_phase_order_for_same_pair() {
        ...
        assert_eq!(g.edges[&key].edge_type, EdgeType::ExplicitDependency,
    ...
    fn no_edges_when_completely_independent_same_phase() {
        let defs = vec![sim("sys_a", vec![6], vec![6]), sim("sys_b", vec![100], vec![100])];
        let g = GraphConstructionLayer::build(&defs).unwrap();
    ...
    fn node_and_edge_order_deterministic() {
        let defs_1 = vec![sim("s_z", vec![],vec![1]), sim("s_a", vec![1],vec![]), sim("s_m", vec![],vec![])];
        let defs_2 = vec![sim("s_m", vec![],vec![]), sim("s_z", vec![],vec![1]), sim("s_a", vec![1],vec![])];
        let g1 = GraphConstructionLayer::build(&defs_1).unwrap();
        let g2 = GraphConstructionLayer::build(&defs_2).unwrap();
        assert_eq!(g1.system_ids(), g2.system_ids(), "Node order must be deterministic (D11)");

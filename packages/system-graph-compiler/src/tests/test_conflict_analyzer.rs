...
mod tests {
    ...
    fn sim(id: &str, reads: Vec<u32>, writes: Vec<u32>) -> SystemDefinition {
        SystemDefinition::with_spec(id, id, PhaseEnum::Simulation.into(), reads, writes)
    }

    fn analyze(
        defs: &[SystemDefinition],
    ) -> crate::conflict_analyzer::conflict_analyzer::ConflictReport {
    ...
    fn raw_hazard_detected_and_forces_serialization() {
        let r = analyze(&[
            sim("writer", vec![], vec![5]),
            sim("reader", vec![5], vec![]),
        ]);
    ...
    fn bidirectional_raw_both_detected() {
        ...
        assert!(
            r.read_write_hazards.len() >= 2,
            "Both RAW directions must be detected"
        );
    }
    ...
    fn transitive_conflicts_same_group() {
        ...
        assert!(
            constrained.iter().any(|g| g.members.len() == 3),
    ...
    fn independent_pairs_in_separate_groups() {
        let r = analyze(&[
            sim("s_a", vec![], vec![1]),
            sim("s_b", vec![], vec![1]), // pair 1
            sim("s_c", vec![], vec![5]),
            sim("s_d", vec![], vec![5]), // pair 2
        ]);

...
use crate::cycle_detection::cycle_diagnostics::CycleDiagnosticReport;
...
use crate::phase_segmentation::phase_segmentation_layer::{PhaseBucket, PhaseSegmentationLayer};
use crate::sgc_pipeline::SgcPipeline;
        ...
        edges: edges
            ...
            .map(|&(from, to)| {
                (
                    ...
                    SystemEdge::explicit_dependency(from, to),
                )
            })
            .collect(),
    }
...
fn raw_chain_is_acyclic() {
    let defs = vec![
        sim("sys_writer", vec![], vec![5]),
        sim("sys_reader", vec![5], vec![]),
    ];
    assert!(check_defs(&defs).is_ok());
}
...
fn two_node_cycle_stage_is_cycle_detection() {
    ...
    assert_eq!(
        CycleDetector::check(&bucket).unwrap_err().stage(),
        "CycleDetection"
    );
}
...
fn disconnected_one_cyclic_one_acyclic_detected() {
    let bucket = PhaseBucket {
        ...
        edges: {
            ...
            m.insert(
                ("sys_p".into(), "sys_q".into()),
                SystemEdge::explicit_dependency("sys_p", "sys_q"),
            );
            m.insert(
                ("sys_r".into(), "sys_s".into()),
                SystemEdge::explicit_dependency("sys_r", "sys_s"),
            );
            m.insert(
                ("sys_s".into(), "sys_r".into()),
                SystemEdge::explicit_dependency("sys_s", "sys_r"),
            );
            m
        },
    };
...
fn cycle_path_starts_at_lex_minimum() {
    ...
    if let Err(CompilationError::Cycle(ce)) = CycleDetector::check(&bucket) {
        assert_eq!(
            ...
            "Normalized cycle must start at lex-smallest (D11)"
        );
    } else {
        panic!("Expected Cycle error");
    }
...
fn diagnostic_strategies_sorted_by_difficulty() {
    ...
    if let Err(CompilationError::Cycle(ce)) = CycleDetector::check(&bucket) {
        ...
        for w in ranks.windows(2) {
            assert!(w[0] <= w[1], "Strategies must be sorted Low→High");
        }
    } else {
        panic!("Expected Cycle error");
    }
...
fn diagnostic_summary_names_systems() {
    let bucket = make_bucket(
        &["sys_alpha", "sys_beta"],
        &[("sys_alpha", "sys_beta"), ("sys_beta", "sys_alpha")],
    );
    if let Err(CompilationError::Cycle(ce)) = CycleDetector::check(&bucket) {
        let report = CycleDiagnosticReport::build(&ce, &bucket);
...
fn cycle_detection_deterministic_same_error_twice() {
    ...
    match (r1, r2) {
        (Err(CompilationError::Cycle(ce1)), Err(CompilationError::Cycle(ce2))) => {
            assert_eq!(
                ce1.cycle_path, ce2.cycle_path,
                "Must be deterministic (D11)"
            );
        }

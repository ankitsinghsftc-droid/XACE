...
mod tests {
    ...
    fn compressor_first_send_passes_all_fields() {
        ...
        c.compress(&mut p);
        assert_eq!(
            p.modified_entities[&1].component_updates[&1]
                .field_changes
                .len(),
            2
        );
    ...
    fn compressor_identical_second_send_eliminates_entity_update() {
        ...
        let make = || {
            ...
            p.add_component_update(
                1,
                WireComponentUpdate::new(1, "T", vec![WireFieldChange::new("x", "1.0")]),
            );
            p
        };
        let mut p1 = make();
    ...
    fn compressor_destroy_evicts_entity_from_cache() {
        ...
        p1.add_component_update(
            1,
            WireComponentUpdate::new(1, "T", vec![WireFieldChange::new("x", "1")]),
        );
        c.compress(&mut p1);
    ...
    fn snapshot_recovery_includes_active_entities() {
        ...
        let snap =
            valid_world_snapshot(1, vec![(1, EntityState::Active), (2, EntityState::Active)]);
        let payload = r
    ...
    fn snapshot_recovery_partial_filters_to_requested_ids() {
        let mut r = SnapshotRecovery::new("0.1.0", 1, 0);
        let mut snap = valid_world_snapshot(
            1,
            vec![
                (1, EntityState::Active),
    ...
    fn sync_engine_identical_field_updates_compressed_to_nothing() {
        ...
        e.process_tick(&delta_with_field(1, 1, "x", r#"{"x":1}"#), &snap)
            .unwrap();
        let result = e
            .process_tick(

...
fn init_world() -> *mut OpaqueWorld {
    ...
    let code = unsafe { xace_init(&mut ptr, 42, 1024 * 1024) };
    assert_eq!(code, 0);
    assert!(!ptr.is_null());
    ptr
}
...
fn xace_version_from_multiple_threads() {
    let handles: Vec<_> = (0..8).map(|_| {
        std::thread::spawn(|| unsafe { xace_version() })
    }).collect();
    for h in handles {
...
fn xace_init_null_out_ptr_returns_null_pointer_error() {
    let code = unsafe { xace_init(std::ptr::null_mut(), 42, 1024 * 1024) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
}
 
#[test]
fn xace_load_cgs_null_world_returns_null_pointer_error() {
    let code = unsafe { xace_load_cgs(std::ptr::null_mut(), MINIMAL_CGS.as_ptr(), MINIMAL_CGS.len() as u32) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
}
...
fn xace_load_cgs_null_json_returns_null_pointer_error() {
    ...
    let code  = unsafe { xace_load_cgs(world, std::ptr::null(), 10) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
    unsafe { xace_shutdown(world) };
}
 
#[test]
fn xace_tick_before_load_cgs_returns_not_initialized() {
    let world = init_world();
    let code  = unsafe { xace_tick(world) };
    assert_eq!(code, XaceErrorCode::NotInitialized as i32,
...
fn xace_load_cgs_twice_returns_already_initialized() {
    let world = init_world();
    let code1 = unsafe { xace_load_cgs(world, MINIMAL_CGS.as_ptr(), MINIMAL_CGS.len() as u32) };
    assert_eq!(code1, 0);
    let code2 = unsafe { xace_load_cgs(world, MINIMAL_CGS.as_ptr(), MINIMAL_CGS.len() as u32) };
...
fn xace_tick_null_world_returns_null_pointer_error() {
    let code = unsafe { xace_tick(std::ptr::null_mut()) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
}
 
#[test]
fn xace_apply_input_null_world_returns_null_pointer_error() {
    let code = unsafe { xace_apply_input(std::ptr::null_mut(), [0u8].as_ptr(), 1) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
}
...
fn xace_apply_input_zero_len_is_noop() {
    let world = init_world();
    unsafe { xace_load_cgs(world, MINIMAL_CGS.as_ptr(), MINIMAL_CGS.len() as u32) };
    let code = unsafe { xace_apply_input(world, [0u8].as_ptr(), 0) };
...
fn xace_get_state_delta_null_buffer_returns_null_pointer_error() {
    let world = init_world();
    unsafe { xace_load_cgs(world, MINIMAL_CGS.as_ptr(), MINIMAL_CGS.len() as u32) };
    unsafe { xace_tick(world) };
    let mut size: u32 = 4096;
    let code = unsafe { xace_get_state_delta(world, std::ptr::null_mut(), &mut size) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
    unsafe { xace_shutdown(world) };
}
...
fn xace_get_state_delta_buffer_too_small_returns_required_size() {
    ...
    let mut size: u32 = 1;
    let code = unsafe { xace_get_state_delta(world, tiny_buf.as_mut_ptr(), &mut size) };
    ...
    if code == XaceErrorCode::BufferTooSmall as i32 {
        assert!(size > 1, "required size must be > 1 when buffer was too small");
    }
    unsafe { xace_shutdown(world) };
}
...
fn xace_shutdown_null_world_returns_null_pointer_error() {
    let code = unsafe { xace_shutdown(std::ptr::null_mut()) };
    assert_eq!(code, XaceErrorCode::NullPointer as i32);
}

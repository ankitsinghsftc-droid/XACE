...
use super::ffi_transport::{FfiSimulation, load_cgs_json};
use super::handle_types::{OpaqueWorld, FfiWorldHandle, box_into_raw, raw_into_box, as_mut};
...
pub extern "C" fn xace_init(
    ...
    ffi_guard!({
        ...
        let delta_size = if delta_buf_bytes == 0 { 4 * 1024 * 1024 } else { delta_buf_bytes };
...
#[no_mangle]
pub extern "C" fn xace_get_tick_number(
    world:    *mut OpaqueWorld,
    out_tick: *mut u64,
) -> i32 {
    ffi_guard!({
...
pub extern "C" fn xace_get_last_error(
    ...
    ffi_guard!({
        ...
        let msg   = if w.last_error.is_empty() { "ok" } else { &w.last_error };

...
use xace_asset_runtime::hot_reload::{
    reload_coordinator::ReloadCoordinator, tick_boundary_gate::TickBoundaryGate,
...
fn coordinator_ignores_untracked_file() {
    ...
    assert!(
        gate.is_empty(),
        "untracked file must not produce a reload request"
    );
}

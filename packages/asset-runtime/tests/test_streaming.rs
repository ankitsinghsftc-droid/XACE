...
impl MockCdn {
    ...
    fn add(&self, uri: &str, data: &[u8]) {
        self.content.lock().unwrap().insert(uri.to_string(), data.to_vec());
    }
 
    fn fail(&self, uri: &str) {
        self.fail_uris.lock().unwrap().insert(uri.to_string());
    }
}
...
impl ICdnAdapter for MockCdn {
    async fn fetch(&self, uri: &str, _range: Option<(u64, u64)>) -> CdnResult<Vec<u8>> {
        if self.fail_uris.lock().unwrap().contains(uri) {
            return Err(CdnError::NotFound { uri: uri.to_string() });
        }
        self.content.lock().unwrap().get(uri)
            .cloned()
            .ok_or_else(|| CdnError::NotFound { uri: uri.to_string() })
    }
 
    async fn content_length(&self, uri: &str) -> CdnResult<Option<u64>> {
        Ok(self.content.lock().unwrap().get(uri).map(|d| d.len() as u64))
    }
...
mod priority_queue_tests {
    ...
    fn same_priority_fifo() {
        let q = AssetPriorityQueue::new(100);
        for i in 0..5usize {
            let req = LoadRequest::new(format!("a{}", i), "uri")
                .with_priority(LoadPriority::Environment);
            q.push(req);
...
fn streamer_loads_asset_and_returns_result() {
    ...
    let config  = StreamerConfig { max_concurrent: 2, ..Default::default() };
    let streamer = AssetStreamer::new(cdn, config);
 
    let req = LoadRequest::new("mesh_knight", "mesh://knight.fbx")
        .with_priority(LoadPriority::Player);
...
fn streamer_cancel_removes_pending_request() {
    ...
    let config  = StreamerConfig { max_concurrent: 0, ..Default::default() }; // no dispatch
    let streamer = AssetStreamer::new(cdn, config);
    ...
    assert!(cancelled > 0 || streamer.queue_depth() == 0,

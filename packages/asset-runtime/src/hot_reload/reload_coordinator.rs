...
use crate::hot_reload::file_watcher::{FileChangeEvent, FileChangeKind, FileWatcher};
...
impl AssetOwnershipMap {
    ...
    pub fn register(
        ...
        self.inner.insert(
            file_path.into(),
            (AssetId::new(asset_id), component_type_id, entity_id, current_version),
        );
    }
 
    pub fn lookup(&self, path: &Path)
        -> Option<(AssetId, u32, u64, u32)>
    {
    ...
    pub fn handle_changes(&self, events: Vec<FileChangeEvent>) {
        for event in events {
            ...
            let content_hash = match VersionHasher::hash_file(&event.path) {
                Ok(h)  => h,
                Err(e) => {
                    eprintln!("[xace-hot-reload] Cannot hash '{}': {}", event.path.display(), e);
                    continue;
                }
            };

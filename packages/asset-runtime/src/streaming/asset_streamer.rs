...
use crate::streaming::load_result::{AssetBytes, LoadResult};
...
impl AssetStreamer {
    ...
    /// Typical use: pass a closure that consumes each ready result and routes
    /// successful loads into the runtime mutation path at the tick boundary.
        ...
        self.runtime.spawn(async move {
            ...
            // Update stats
            {
                ...
                if let Some(b) = result.bytes() {
                    s.total_bytes += b.size_bytes as u64;
                }
                // Rolling avg latency
                let n = s.total_completed as f64;
                s.avg_latency_ms =
                    s.avg_latency_ms * ((n - 1.0) / n) + (duration.as_millis() as f64 / n);
            }
    ...
    async fn fetch_asset(
        ...
        let data =
            tokio::time::timeout(timeout, cdn.fetch(&request.source_uri, request.byte_range))
                .await
                .map_err(|_| {
                    format!(
                        "Timeout fetching '{}' after {}s",
                        request.source_uri,
                        timeout.as_secs()
                    )
                })?

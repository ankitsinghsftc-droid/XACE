...
impl CloudFrontAdapter {
    ...
    pub fn new(base_url: impl Into<String>, api_key: Option<String>) -> Result<Self, CdnError> {
        ...
        Ok(Self { client, base_url: base, api_key })
    }
...
impl ICdnAdapter for CloudFrontAdapter {
    async fn fetch(
        ...
        let response = req.send().await.map_err(|e| CdnError::Network(e.to_string()))?;
        ...
        if status == 401 || status == 403 {
            return Err(CdnError::AuthError(
                format!("CloudFront auth error (HTTP {}). Check XACE_CDN_API_KEY.", status)
            ));
        }
        if !status.is_success() && status != 206 {
            let body = response.text().await.unwrap_or_default();
            return Err(CdnError::Http { status: status.as_u16(), body });
        }
    ...
    fn adapter_name(&self) -> &'static str { "cloudfront" }
}

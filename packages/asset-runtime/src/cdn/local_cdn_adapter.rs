...
impl LocalCdnAdapter {
    ...
    pub fn from_env() -> Self {
        let root = std::env::var("XACE_LOCAL_CDN_ROOT")
    ...
    fn resolve_path(&self, uri: &str) -> Result<PathBuf, CdnError> {
        ...
        if !canonical_path.starts_with(&canonical_root) {
            return Err(CdnError::InvalidUri(format!(
                "Path traversal attempt rejected for URI: {}", uri
            )));
        }
...
impl ICdnAdapter for LocalCdnAdapter {
    async fn fetch(
        ...
        let mut file = tokio::fs::File::open(&path).await
            .map_err(|_| CdnError::NotFound { uri: uri.to_string() })?;
    ...
    fn adapter_name(&self) -> &'static str { "local" }
}
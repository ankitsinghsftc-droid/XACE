...
pub fn run(args: RunArgs, ctx: &Context) -> Result<i32, CliError> {
    ...
    let status = std::process::Command::new("cargo")
        .args([
            "run", "--quiet", "--package", "xace-engine-adapter", "--",
            "--engine", &args.engine,
            "--host",   host,
            "--port",   &port.to_string(),
        ])
        ...
        .map_err(|e| CliError::BuildError {
            stage: "engine-adapter".to_string(), message: e.to_string(),
        })?;
...
fn test_tcp_connection(host: &str, port: u16) -> Result<(), std::io::Error> {
    ...
    TcpStream::connect_timeout(
        &addr.parse().map_err(|_| std::io::Error::new(std::io::ErrorKind::AddrNotAvailable, "bad addr"))?,

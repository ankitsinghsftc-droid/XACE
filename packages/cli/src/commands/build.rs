...
pub fn run(args: BuildArgs, ctx: &Context) -> Result<i32, CliError> {
    ...
    if !args.skip_validation {
        ...
        let result = bridge.invoke_module(
            "xace_gde",
            "validate",
    ...
    let compile_result = bridge.invoke_module(
        "xace_gde",
        "compile",
        &["--game-dir", &args.game.display().to_string()],
    )?;
    ...
    let output_dir = args
        ...
        .as_ref()
        .cloned()
        .unwrap_or_else(|| config.output_dir());
...
fn generate_artifact(
    ...
    match target {
        TargetEngine::Unity | TargetEngine::Godot | TargetEngine::Unreal => {
            ...
            let manifest_path = output_dir.join(format!("xace_{}_config.json", target.to_string()));
        ...
        TargetEngine::Standalone => Err(CliError::not_implemented("standalone target", "Phase 17")),
    }
}

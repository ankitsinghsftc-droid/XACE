...
pub fn run(args: TestArgs, ctx: &Context) -> Result<i32, CliError> {
    ...
    finish_tests(total_passed, total_failed, total_ignored, start, args.report.as_deref(), ctx)
}
...
fn run_cargo_tests(
    ...
    if passed > 0 || failed == 0 {
        ctx.print_ok(&format!("{} passed, {} failed, {} ignored", passed, failed, ignored));
    } else {
...
fn run_pytest(fail_fast: bool, ctx: &Context) -> (u32, u32) {
    ...
    let text = String::from_utf8_lossy(&output.stdout).to_string();
    if ctx.verbose {
        for line in text.lines() { ctx.verbose_log(line); }
    }
...
fn extract_number(text: &str, keyword: &str) -> u32 {
    ...
    before.split_whitespace().last()?.parse().ok().unwrap_or(0)
}
 
// note: extract_number returns Option internally but we use `?` with Option — needs fix
// correcting with a standalone helper
fn extract_number(text: &str, keyword: &str) -> u32 {
...
fn finish_tests(
    ...
    if !ctx.json {
        ...
        if ok {
            println!(
                ...
                passed, ignored, elapsed.as_secs_f64()
            );
        } else {
            println!(
                "\x1b[31m✗ {} test(s) failed\x1b[0m — {} passed, {} failed, {} ignored in {:.2}s",
                failed, passed, failed, ignored, elapsed.as_secs_f64()
            );
        }
    }
    ...
    if let Some(path) = report {
        std::fs::write(path, serde_json::to_string_pretty(&report_json)?)
            .map_err(|e| CliError::Io { path: Some(path.to_path_buf()), source: e })?;

...
impl StorageRouter {
    ...
    pub fn select_from_yaml(yaml_text: &str) -> (StorageStrategy, StorageConfig) {
        ...
        struct RuntimeSection {
            #[serde(default)]
            expected_max_entities: Option<usize>,
            #[serde(default)]
            storage_backend_threshold: Option<usize>,
            #[serde(default)]
            storage_backend: Option<String>,
        }

        let parsed: GameConfig =
            serde_yaml::from_str(yaml_text).unwrap_or(GameConfig { runtime: None });

        let rt = parsed.runtime;
        let mut config = StorageConfig::default();

        if let Some(rs) = rt {
            if let Some(n) = rs.expected_max_entities {
                config.expected_max_entities = n;
            }
            if let Some(n) = rs.storage_backend_threshold {
                config.entity_threshold = n;
            }

...
#[cfg(test)]
mod test_trace {
    use xace_observability::trace::{Span, SpanId, SpanStatus, TraceId, TraceLog};
    use xace_observability::tracer::{enter_span, current_trace_id, TRACER};
    ...
    fn span_with_attributes_serialises() {
        ...
        let record = span.close();
        assert_eq!(record.attributes.get("key").map(|s| s.as_str()), Some("value"));
